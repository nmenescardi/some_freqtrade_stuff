...
class CombinedBinHAndClucV6H(IStrategy):
    ...
    buy_params = {
        "buy_cluc_close_bblowerband": 0.445,
        "buy_cluc_volume": 28,
        "buy_long_rsi_diff": 41.426,
        "buy_bin_bbdelta_close":  0.007,
        "buy_bin_closedelta_close": 0.0108,
        "buy_bin_enable": True,
        "buy_bin_guard": True,
        "buy_bin_tail_bbdelta": 0.622,  
    ...
    sell_params = {
        ...
        "sell_roi_override_rsi_threshold": 58,
    }
    ...
    # bin buy parameters
    buy_bin_bbdelta_close =  DecimalParameter(0.0, 0.05, default=0.007, space='buy', optimize=cust_optimize['buy_bin_bbdelta_close'], load=True)
    buy_bin_closedelta_close = DecimalParameter(0.0, 0.03, default=0.0108, decimals=4, space='buy', optimize=cust_optimize['buy_bin_closedelta_close'], load=True)
    buy_bin_tail_bbdelta = DecimalParameter(0.0, 1.0, default=0.622, decimals=3, space='buy', optimize=cust_optimize['buy_bin_tail_bbdelta'], load=True)
    buy_bin_guard = CategoricalParameter([True, False], default=True, space='buy', optimize=cust_optimize['buy_bin_guard'], load=True)

    # cluc buy parameters
    buy_cluc_close_bblowerband = DecimalParameter(0.0, 1.5, default=0.445, decimals=3, space='buy', optimize=cust_optimize['buy_cluc_close_bblowerband'], load=True)
    ...
    # log buy parameters
    buy_long_rsi_diff = DecimalParameter(40, 45, default=41.426, decimals=3, space='buy', optimize=cust_optimize['buy_long_rsi_diff'], load=True)
    ...
    sell_roi_override_rsi_threshold = IntParameter(40, 70, default=58, space='sell', optimize=cust_optimize['sell_roi_override_rsi_threshold'], load=True)
