...
class CombinedBinHAndClucV7_OPT(IStrategy):
    INTERFACE_VERSION = 2
    ...
    buy_bb40_bbdelta_close = DecimalParameter(0.005, 0.04, default=0.013, space='buy', optimize=True, load=True)
    buy_bb40_closedelta_close = DecimalParameter(0.01, 0.03, default=0.027, space='buy', optimize=True, load=True)
    buy_bb40_tail_bbdelta = DecimalParameter(0.2, 0.4, default=0.317, space='buy', optimize=True, load=True)

    buy_bb20_close_bblowerband = DecimalParameter(0.8, 1.1, default=0.873, space='buy', optimize=True, load=True)
    buy_bb20_volume = IntParameter(18, 36, default=19, space='buy', optimize=True, load=True)

    buy_rsi_diff = DecimalParameter(34.0, 60.0, default=56.46, space='buy', decimals=2, optimize=True, load=True)

    buy_min_inc = DecimalParameter(0.005, 0.05, default=0.01, space='buy', decimals=2, optimize=True, load=True)
    buy_rsi_1h = DecimalParameter(40.0, 70.0, default=68.34, space='buy', decimals=2, optimize=True, load=True)
    buy_rsi = DecimalParameter(30.0, 40.0, default=39.6, space='buy', decimals=2, optimize=True, load=True)
    buy_mfi = DecimalParameter(36.0, 65.0, default=50.39, space='buy', decimals=2, optimize=True, load=True)
    ...
    sell_rsi_main = DecimalParameter(72.0, 90.0, default=72.22, space='sell', decimals=2, optimize=True, load=True)
